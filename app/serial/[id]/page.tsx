...
export async function generateMetadata({ params }: Props) {
    ...
    return {
        title: item ? `${item.title} — TOPKino.NET` : "Topilmadi",
    };
}
...
export default async function SerialDetailPage({ params }: Props) {
    ...
    // Generate episode list for the first season
    const episodeCount = Math.ceil(item.episodes / item.seasons);
    const episodes = Array.from({ length: Math.min(episodeCount, 12) }, (_, i) => i + 1);
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <div className="flex-1 pt-2">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                            ...
                            <span className="text-xs bg-purple-500/20 text-purple-400 border border-purple-500/30 rounded-full px-3 py-0.5">
                                Serial
                            </span>
                ...
                <div className="mt-10">
                    ...
                    <div className="relative w-full rounded-2xl overflow-hidden bg-[#16161e] border border-white/5 shadow-2xl"
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                            ...
                            <p className="text-gray-500 text-sm">Serial yuklanmoqda...</p>
                        </div>
                ...
                <div className="mt-10">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {episodes.map((ep) => (
                            <button
                                key={ep}
                                className={`rounded-xl py-3 text-sm font-semibold transition-all duration-200 border ${ep === 1
                                        ? "bg-orange-500 border-orange-500 text-white shadow-lg shadow-orange-500/30"
                                        : "bg-[#16161e] border-white/5 text-gray-400 hover:border-orange-500/50 hover:text-white"
                                    }`}
                            >
                                {ep}-qism
                            </button>
